...
CREATE TABLE `don_hang` (
  ...
  `phuong_thuc_thanh_toan` ENUM('tien_mat', 'chuyen_khoan', 'momo', 'vnpay', 'vietqr') NOT NULL DEFAULT 'tien_mat',
  `trang_thai_thanh_toan` ENUM('chua_thanh_toan', 'da_thanh_toan', 'hoan_tien') NOT NULL DEFAULT 'chua_thanh_toan',
  `trang_thai_don_hang` ENUM('cho_xac_nhan', 'dang_che_bien', 'san_sang_giao', 'dang_giao', 'da_giao', 'da_huy') NOT NULL DEFAULT 'cho_xac_nhan',
  `vi_do_giao` DECIMAL(10, 8) DEFAULT NULL COMMENT 'Vĩ độ GPS nơi giao hàng',
  `kinh_do_giao` DECIMAL(11, 8) DEFAULT NULL COMMENT 'Kinh độ GPS nơi giao hàng',
  `khoang_cach_km` DECIMAL(6, 2) DEFAULT NULL COMMENT 'Khoảng cách giao hàng tính từ mốc quán (km)',
